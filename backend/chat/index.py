...
def handler(event: dict, context) -> dict:
    ...
    if method == "POST" and action == "create_chat":
        ...
        member_ids = [int(x) for x in body.get("member_ids", [])]

        if not name:
            return {"statusCode": 400, "headers": CORS, "body": json.dumps({"error": "name required"})}

        conn = get_conn()
        # Добавляем инициатора (из сессии) в список участников
        my_id = get_user_id_from_session(session_id, conn) if session_id else None
        if my_id and my_id not in member_ids:
            member_ids.insert(0, my_id)

        cur = conn.cursor()

        # Для личного чата — ищем существующий где оба участника уже есть
        if not is_group and len(member_ids) == 2:
            uid_a, uid_b = member_ids[0], member_ids[1]
            cur.execute(f"""
